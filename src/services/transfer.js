...
module.exports = (app) => {
  const find = async (filter = {}) => {
    return await app.db("transfers").where(filter).select();
  };

  const findOne = async (filter = {}) => {
    return await app.db("transfers").where(filter).first();
  };
  ...
  const validate = async (transfer) => {
    ...
    if (transfer.acc_ori_id === transfer.acc_dest_id) {
      throw new ValidationError("As contas devem ser diferentes");
    }
